...
import numpy as np
from PyQt6.QtCore import QDateTime, Qt, QTime, QTimer, pyqtSignal
from PyQt6.QtWidgets import (QApplication, QColorDialog, QComboBox, QDialog,
                             QHBoxLayout, QHeaderView, QLabel, QLineEdit,
                             QMainWindow, QPushButton, QTableWidget,
                             QTableWidgetItem, QVBoxLayout)

from ui.modify_dialog import ModifyDialog
from ui.options_dialog import OptionsDialog
from ui.timer_view import Ui_MainWindow

from model.database import create_tables, add_solve, get_solves
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.db_connection = sqlite3.connect("database/database.db")
    ...
    def setup_table(self):
        ...
        self.table_previous_times.setHorizontalHeaderLabels(["id", "Time (s)", "Date"])
    ...
    def load_sessions(self):
        ...
        if not rows:
            ...
            for i in range(1, 6):
                self.cursor.execute("INSERT INTO sessions(name) VALUES (?)", (str(i),))
            self.db_connection.commit()
    ...
    def load_saved_solves(self):
        ...
        self.all_solves = [
            row[1] if isinstance(row[1], (int, float)) else float("inf") for row in rows
        ]
        ...
        # Refresh status bar
        fastest_str = (
            f"{self.fastest_time:.3f}" if self.fastest_time != float("inf") else "N/A"
        )
    ...
    def start_timer(self):
        ...
        if not self.running:
            ...
            if self.solves_count > 0:
                ao5_str = (
                    f"{self.best_ao5:.3f}" if self.best_ao5 != float("inf") else "N/A"
                )
                ao12_str = (
                    f"{self.best_ao12:.3f}" if self.best_ao12 != float("inf") else "N/A"
                )
                fastest_str = (
                    f"{self.fastest_time:.3f}"
                    if self.fastest_time != float("inf")
                    else "N/A"
                )
                self.statusBar().showMessage(
                    f"Fastest: {fastest_str} s - Best ao5: {ao5_str} s - Best ao12: {ao12_str} s"
                )
            else:
                self.statusBar().showMessage("Do your first solve")
    ...
    def new_session(self):
        ...
        if ok and name:
            try:
                self.cursor.execute("INSERT INTO sessions(name) VALUES (?)", (name,))
                self.db_connection.commit()
            except sqlite3.IntegrityError:
                QMessageBox.warning(
                    self, "Duplicate", f"Session “{name}” already exists."
                )
                return
            self.comboBox_session.addItem(name)
    ...
    def delete_session(self):
        ...
        # remove from DB
        self.cursor.execute("DELETE FROM solves WHERE Session = ?", (session,))
        self.cursor.execute("DELETE FROM sessions WHERE name = ?", (session,))
        self.db_connection.commit()
        ...
        if self.comboBox_session.count() == 0:
            # recreate a default
            self.cursor.execute("INSERT INTO sessions(name) VALUES (?)", ("Default",))
            self.db_connection.commit()
    ...
    def save_time(self):
        ...
        if time < self.fastest_time:
            ...
            self.statusBar().showMessage(
                f"New fastest time: {self.fastest_time} seconds"
            )
        ...
        if self.solves_count > 5:
            ...
            if ao5 < self.best_ao5:
                self.best_ao5 = ao5
                self.statusBar().showMessage(
                    f"New best AO5: {self.best_ao5:.3f} seconds"
                )
        if self.solves_count > 12:
            ...
            if ao12 < self.best_ao12:
                self.best_ao12 = ao12
                self.statusBar().showMessage(
                    f"New best AO12: {self.best_ao12:.3f} seconds"
                )
    ...
    def modify_time(self):
        ...
        if last_index >= 0:
            if modification == "+2":
                self.cursor.execute(
                    "UPDATE solves SET Time = Time + 2 WHERE id = ?",
                ...
                self.cursor.execute(
                    'UPDATE solves SET Time = "DNF" WHERE id = ?',
                    (self.table_previous_times.item(last_index, 0).text(),),
                )
                self.db_connection.commit()
